...
## Object Transform system (task 2.1)

Scene nodes now carry an editable local **Transform** (translate / rotate / scale), and a runtime scene graph computes each node's world matrix from its parent chain.

- **Coordinate system**: **Z-up right-handed** world (`+X` east, `+Y` north, `+Z` up). Rotations are in **degrees** with X→Y→Z order (`M = T * Rz * Ry * Rx * S`). Units are model-native (no metric conversion).
- **glTF is Y-up**: the model-root node rotates `+90°` about X to bring assets into the Z-up world. OBJ is treated the same way. The renderer's display transform applies a matching `-90°`, so the on-screen orientation is unchanged while world / lighting / display axes are all Z-up.
- **Two spaces**: vertices upload in each mesh node's **local** space (node transforms are no longer baked at import). The scene root carries the normalization matrix (center + scale) — this **normalized scene space** is the shared shading / shadow / measurement space.
- **Per-object matrix**: `Object::GetWorldMatrix()` is pushed per draw (Vulkan vertex push constant; OpenGL `uObjectModel` uniform). The main, transparent, and shadow passes all apply it, so **shadows and world-coordinate readback follow transformed objects**.
